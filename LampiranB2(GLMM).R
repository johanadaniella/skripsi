...
#GLMM Negative Binom CLAIM FREQ
library(MASS)
...
bnm = glmer(ClaimOcc ~ (1|VehBody)+DrivAge, family = binomial(link = "logit"), data = training, nAGQ = 25, offset = log(Exposure))
summary(bnm)
...
inversegaus = glmer(ClaimAmountBaru ~ (1|VehBody)+VehAge+Gender+DrivAge, family = inverse.gaussian(link = "identity"), data = training01, offset = log(Exposure))
summary(inversegaus)
...
inversegaus1 = glmer(ClaimAmount ~ (1|VehBody)+VehAge+Gender+DrivAge, family = inverse.gaussian(link = "identity"), data = training01, offset = log(Exposure), control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 10000)))
