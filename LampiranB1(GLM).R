...
#subset data untuk pemodelan frekuensi klaim  
claim <- ausprivauto0405[ausprivauto0405$ClaimAmountBaru <= 500, ]
#Pembagian data 
set.seed(777)
train_index <- createDataPartition(claim$ClaimNb, p = 0.8, list = FALSE)
training <- claim[train_index, ]
testing <- claim[-train_index, ]
...
library(MASS)
nbfreq = glm.nb(ClaimNb~VehValue+VehAge+VehBody+Gender+DrivAge, link = log, data = training)
summary(nbfreq)
...
invgs1 = glm(ClaimAmount~VehAge+VehBody+Gender+DrivAge, family = inverse.gaussian(link = "identity"), data = training01, offset = log(Exposure))
summary(invgs1)
